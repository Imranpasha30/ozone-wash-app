/**
 * LiveStreamScreen — placeholder while live broadcast is not yet available.
    ...
    <View style={s.root}>
      ...
      <View style={s.body}>
        <View style={s.iconHalo}>
          <Play size={44} weight="fill" color={C.primary} />
        </View>
        <View style={s.soonBadge}>
          <Text style={s.soonBadgeText}>COMING SOON</Text>
        </View>
        <Text style={s.title}>Live broadcast for crews</Text>
        <Text style={s.sub}>
          Stream the cleaning service to the customer in real time, so they can verify every step as it happens. We're rolling this out shortly.
        </Text>

        <TouchableOpacity onPress={() => navigation.goBack()} style={s.btn} activeOpacity={0.85}>
          <Text style={s.btnText}>Back to job</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
